...
@app.route('/')
def index():
    return "Welcome to the Real Estate Ledger on Peer 3!"
...
def add_transaction():
    ...
    latest_block_hash = conn.getLastBlock(table_name="peer3_ledger")
    ...
    if(new_block['prev_block_hash'] == latest_block_hash):
        conn.saveToDB((new_block['id'], new_block['prev_block_hash'], new_block['curr_block_hash'], new_block['transaction_details']), table_name="peer3_ledger")
...
def update_change():
    ...
    conn.saveToDB((new_block['id'], new_block['prev_block_hash'], new_block['curr_block_hash'], new_block['transaction_details']), table_name="peer3_ledger", transType="update")
    
    return {"response": "Successfully updated in Peer 3."}
